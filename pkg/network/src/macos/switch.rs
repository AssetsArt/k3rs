...
pub struct MacSwitch {
    ...
    utun: Option<Arc<crate::macos::utun::Utun>>,
}

impl Drop for MacSwitch {
    fn drop(&mut self) {
        if let Some(ref utun) = self.utun {
            crate::macos::pfnat::teardown_nat(&utun.name, pkg_constants::network::DEFAULT_POD_CIDR);
        }
    }
}
...
impl MacSwitch {
    ...
    pub fn new(dns_port: u16) -> Self {
        ...
        let utun = match crate::macos::utun::Utun::create() {
            Ok(u) => {
                ...
                if let Err(e) = crate::macos::pfnat::setup_nat(
                    &u.name,
        ...
        Self {
            ...
            dns_addr: SocketAddr::new(std::net::IpAddr::V4(Ipv4Addr::LOCALHOST), dns_port),
            utun,
        }
    }
    ...
    pub async fn add_vm(
        ...
        info!(
            "[switch] registered VM {} (ip={}, vpc={})",
            id, guest_ipv4, vpc_id
        );
        Ok(())
    }
            ...
            loop {
                ...
                for id in readable_ids {
                    ...
                    match ethertype {
                        ...
                        _ => {
                            trace!(
                                "[switch] dropping frame with ethertype 0x{:04x} from {}",
                                ethertype, id
                            );
                        }
                    }
    ...
    async fn handle_arp(&self, src_id: &str, frame: &[u8]) {
        ...
        debug!(
            "[switch] ARP request from {} ({}) for {}",
            src_id, sender_ip, target_ip
        );
        ...
        if let Some(ep) = eps.get(src_id) {
            ...
            if ret < 0 {
                warn!(
                    "[switch] failed to send ARP reply to {}: {}",
                    src_id,
    ...
    async fn handle_ipv4(&self, src_id: &str, frame: &[u8]) {
        ...
        if let Some(ref dst_id) = dst_id {
            ...
            if let Some(dst_ep) = eps.get(dst_id.as_str()) {
                ...
                if dst_ep.vpc_id != src_vpc_id {
                    trace!(
                        "[switch] VPC isolation: dropping {} → {} (vpc {} → {})",
                        src_id, dst_id, src_vpc_id, dst_ep.vpc_id
                    );
                    return;
                }
                ...
                if ret < 0 {
                    warn!(
                        "[switch] failed to forward to {}: {}",
                        dst_id,
                        io::Error::last_os_error()
                    );
                } else {
                    trace!(
                        "[switch] forwarded {} bytes {} → {}",
                        fwd_frame.len(),
                        src_id,
                        dst_id
                    );
        ...
        if let Some(ref utun) = self.utun {
            ...
            if ret < 0 {
                warn!("[switch] utun write failed: {}", io::Error::last_os_error());
            } else {
                trace!(
    ...
    async fn handle_dns(&self, src_id: &str, frame: &[u8]) {
        ...
        debug!(
            "[switch] DNS query from {} (port {}), {} bytes",
            src_id,
            src_port,
            dns_payload.len()
        );
        ...
        if ret < 0 {
            warn!(
                "[switch] failed to send DNS response to {}: {}",
                src_id,
                io::Error::last_os_error()
            );
        } else {
            debug!(
                "[switch] DNS response sent to {} ({} bytes)",
                src_id,
                resp_frame.len()
            );
        }

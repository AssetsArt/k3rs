...
impl NetworkEnforcer for NftManager {
    ...
    async fn install_netkit_rules(
        ...
        // nftables enforcer: netkit rules use the same pattern as TAP rules (no SIIT)
        self.install_tap_rules(nk_name, guest_ipv4, vpc_id).await
    }

    async fn remove_netkit_rules(&mut self, nk_name: &str) -> Result<()> {
        self.remove_tap_rules(nk_name).await
    }

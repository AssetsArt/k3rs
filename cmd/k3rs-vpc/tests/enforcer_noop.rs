...
async fn test_noop_enforcer_vpc_lifecycle() {
    ...
    assert!(enforcer.ensure_vpc(1, "10.0.1.0/24").await.is_ok()); // idempotent
...
#[tokio::test]
async fn test_noop_enforcer_netkit_rules() {
    let mut enforcer = NoopEnforcer::new();
    enforcer.init().await.unwrap();
    enforcer.ensure_vpc(1, "10.0.1.0/24").await.unwrap();
    ...
    assert!(enforcer.remove_netkit_rules("nk-abc12345").await.is_ok());
}
...
async fn test_noop_enforcer_full_lifecycle() {
    ...
    // Install TAP rules (replaces install_pod_rules for VMs)
    enforcer
        .install_tap_rules(
            "tap-vma1",
            "10.0.1.10",
    ...
    // Teardown
    enforcer.remove_tap_rules("tap-vma1").await.unwrap();
    enforcer.remove_tap_rules("tap-vma2").await.unwrap();
    enforcer.remove_tap_rules("tap-vmb1").await.unwrap();

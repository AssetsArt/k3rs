    ...
    match &cli.command {
        ...
        Commands::Backup { action } => match action {
            ...
            BackupAction::Inspect { file } => {
                ...
                println!("  Version:      {}", backup["version"].as_str().unwrap_or("-"));
                println!(
                    "  Created at:   {}",
            ...
            BackupAction::Status => {
                ...
                println!("Backup status:  {}", status["status"].as_str().unwrap_or("-"));
                println!(
                    "Last backup:    {}",
        ...
        Commands::Restore {
            ...
            if status.is_success() {
                if *dry_run {
                    ...
                    println!("  Entries to restore: {}", result["would_restore"].as_u64().unwrap_or(0));
                    println!("  Backup created at:  {}", result["backup_created_at"].as_str().unwrap_or("-"));
                    println!("  Backup version:     {}", result["backup_version"].as_str().unwrap_or("-"));
                } else {
                    println!("Restore completed:");
                    println!("  Entries imported:   {}", result["imported"].as_u64().unwrap_or(0));
                    println!("  Backup created at:  {}", result["backup_created_at"].as_str().unwrap_or("-"));
                }
            } else {
                eprintln!(
